...
- 引用计数、资源作用域，以及句柄或作用域的策略化释放；首期提供立即和延时策略，后续可扩展下一帧、空闲时或内存压力释放。
...
- UI 路由集中批量注册，动画策略优先使用层级默认预设，特殊界面再单独覆盖。
- 界面打开、关闭、返回栈、互斥和缓存策略。
- View 与 Presenter/ViewModel 的生命周期约定。
- Loading、Toast、确认框等基础组件。
- 刘海屏、安全区、横竖屏和分辨率适配。
- 点击防抖、输入锁和可等待的开关过渡动画；首版支持 Animation 和缩放/透明度 Tween，具体表现通过策略扩展，不写死在 UI 管理器中。
- UI 资源随界面作用域自动释放。
...
当前已实现首版文本本地化能力：系统语言识别、JSON/内存语言包、语言切换与回退、参数插值、基础复数形式和缺失键检查。图片、字体、音频本地化以及语言包独立 Bundle 下载，待项目资源和发布方式明确后接入。
...
当前首轮已完成 UI、数据、存储、音频、网络和 Web Mobile 平台模块的服务骨架，并补充 lifecycle 与 i18n 首版能力、统一门面接入与生命周期清理。lifecycle 已统一分发前后台和场景切换通知，并驱动 BGM 暂停恢复；i18n 已支持文本语言包、回退、参数和基础复数形式。实际登录/大厅 Prefab、真实接口协议、项目配置和本地化资源将在对应业务内容确定后接入，本阶段仍在实施中。
